...
/*   Updated: 2018/06/11 19:02:50 by oshvorak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	switcher(t_game *game, int k, int flag)
{
	if (flag == 1)
	{
...
static	void	print_frame(WINDOW *stdscr, int speed)
{
	int i;
	int j;
...
static	char	*prog_name(char *name)                   // фігова ідея, тому що спочатку ім'я не малочиться
{
	char *res;
...
static	void	speed(t_game *game, int y)
{
	int i;
	int j;
	int num;
...
static	int		print_players(t_game *game, int pn, int y)
{
	int i;
...
static	void	print_info(t_game *game, int y)
{
...
static	void	right_menu(t_game *game)
{
	int pn;
	int y;
