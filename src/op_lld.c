...
static	unsigned int	ret_t_reg(t_game *game, t_process *process)
{
	...
	if (ret_arg(game->area[(process->pc + 1) % MEM_SIZE].value, MASK_1, 6) == T_DIR)
	{
		...
		if (check_reg_ind(game, process, (unsigned char)t_reg))
	...
	else
	{
		...
		arg_ind = write_4_bytes(game, (process->pc + (short)t_ind) % MEM_SIZE);
...
void					op_lld(t_game *game, t_process *process)
{
	...
	if (check_reg_ind(game, process, t_reg) && game->er == 0)
